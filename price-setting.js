/* PRICE_SETTING_SHOPEE_V5_DIRECT_CALC_MULTI_FILE_20260523
 * NNV Marketing System - TMĐT > Thiết lập giá > Shopee
 * Version: V5 Direct Calculator + Multi File + Warning Apply
 */
(function () {
  "use strict";

  var VERSION_MARKER = "PRICE_SETTING_SHOPEE_V5_DIRECT_CALC_MULTI_FILE_20260523";
  var MODULE_KEY = "NNV_PRICE_SETTING_SHOPEE_V5_CONFIG";
  ...
  function renderFilesArea() {
    ...
    state.files.forEach(function (f, index) {
      ...
      if (s && s.warning > 0) {
        warningButton =
          '<button class="ps-btn warn" onclick="window.psApplyAllWarnings(\'' + f.id + '\')">Áp dụng tất cả sản phẩm cảnh báo</button>';
      }
      ...
      html +=
        '<div class="ps-file-card">' +
          '<div class="ps-file-head">' +
            ...
            '</div>' +
            '<button class="ps-icon-btn" title="Xóa file" onclick="window.psRemoveFile(\'' + f.id + '\')">×</button>' +
          '</div>' +
          summaryHtml +
          renderWarningsTable(f) +
          '<div class="ps-file-actions">' +
            '<button class="ps-btn secondary" onclick="window.psDownloadPriceFile(\'' + f.id + '\')">Tải file giá %</button>' +
            '<button class="ps-btn secondary" onclick="window.psDownloadDiscountFile(\'' + f.id + '\')">Tải file chiết khấu</button>' +
            '<button class="ps-btn secondary" onclick="window.psDownloadCheckFile(\'' + f.id + '\')">Tải file kiểm tra</button>' +
            warningButton +
          '</div>' +
        '</div>';
    });
  ...
  function renderUI() {
    ...
    container.innerHTML =
      '<div class="ps-shell">' +
        ...
        '<div class="ps-panel">' +
          '<div class="ps-panel-title">' +
            '<h3>2. Công cụ tính giá trực tiếp</h3>' +
            '<span>Tính nhanh 1 sản phẩm trước khi xử lý file</span>' +
          '</div>' +
          '<div class="ps-grid">' +
        ...
        '<div class="ps-panel">' +
          '<div class="ps-panel-title">' +
            '<h3>3. Nhập file giá gốc Shopee</h3>' +
  ...
  window.psDownloadPriceFile = downloadPriceFile;
  window.psDownloadDiscountFile = downloadDiscountFile;
  window.psDownloadCheckFile = downloadCheckFile;
