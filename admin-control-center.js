...
(function installMktAdminOpsV289(){
    ...
    function mount(){
        ...
        if(!document.getElementById('mkt-admin-ops-nav-v289')){
            ...
            nav.innerHTML='<div class="mkt-ops-head-v289"><div><h2>Trung tâm điều hành</h2><p>Kiểm soát quyền, sức khỏe hệ thống, nhật ký, tìm kiếm và thông báo từ một nơi.</p></div><span class="mkt-ops-admin-chip-v289">QUẢN TRỊ</span></div>'+
                ...
                '<button class="mkt-ops-tab-v289" data-mkt-ops-tab-v289="search">Tìm kiếm</button>'+
                '<button class="mkt-ops-tab-v289" data-mkt-ops-tab-v289="announcements">Tạo thông báo</button></div>';
    ...
    function switchTab(tab){
        if(['permissions','health','audit','search','announcements'].indexOf(tab)===-1)tab='permissions';
    ...
    function syncView(){
        ...
        if(state.active==='announcements'){
            if(window.MKTSystemAnnouncementCenterV1&&typeof window.MKTSystemAnnouncementCenterV1.renderAdmin==='function') window.MKTSystemAnnouncementCenterV1.renderAdmin(view);
            else view.innerHTML='<div class="mkt-audit-empty-v289">Đang tải Trung tâm thông báo...</div>';
        }
    ...
    function boot(){
        ...
        setInterval(function(){var p=adminPage();if(p&&p.classList.contains('active')&&!document.getElementById('mkt-admin-ops-nav-v289'))mount()},1400);
    }
