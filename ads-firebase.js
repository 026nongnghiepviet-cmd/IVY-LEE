/**
 * ADS MODULE V80 (ULTIMATE STABLE)
 * - Khôi phục 100% bộ đọc file (Parser) ổn định của V69.
 * - Khóa chặt chỉ cho phép up file "Nhóm quảng cáo".
 * - Giữ lại giao diện Bảng chuẩn chỉnh và 4 Biểu đồ phân tích.
 * - Đảm bảo mọi hàm xử lý số liệu được nạp đầy đủ.
 */
...
function initAdsAnalysis() {
    console.log("Ads Module V80 Loaded");
    db = getDatabase();
...
// V80: BỘ QUÉT TỐI THƯỢNG (ỔN ĐỊNH NHƯ V69 NHƯNG MẠNH MẼ HƠN)
function parseDataCore(rows) { 
    ...
    for (let i = 0; i < Math.min(rows.length, 15); i++) { 
        ...
        // CHẶN NGAY FILE CHIẾN DỊCH
        if (rowStr.includes("tên chiến dịch") && !rowStr.includes("tên nhóm quảng cáo")) {
            throw new Error("❌ LỖI: Vui lòng xuất báo cáo theo cấp độ NHÓM QUẢNG CÁO! (Không dùng file Chiến dịch)");
        }

        // TÌM HEADER CHÍNH XÁC DỰA VÀO CHỮ "TÊN NHÓM" VÀ "SỐ TIỀN"
        if (rowStr.includes("tên nhóm") && (rowStr.includes("số tiền") || rowStr.includes("amount"))) { 
            ...
            row.forEach((cell, idx) => { 
                ...
                // Gán index duy nhất 1 lần (Lấy cột đầu tiên khớp)
                if (txt.includes("tên nhóm")) { if(colNameIdx === -1) colNameIdx = idx; }
    ...
    for (let i = headerIndex + 1; i < rows.length; i++) { 
        ...
        parsedData.push({ 
            ...
            spend: spend, result: result, clicks: clicks, impressions: imps, messages: msgs,
...
function applyFilters() {
    ...
    if(CURRENT_TAB === 'performance' || CURRENT_TAB === 'finance') {
        ...
        if(pSpend) {
            ...
            const elLeads = document.getElementById('perf-leads');
            if (elLeads) elLeads.innerText = new Intl.NumberFormat('vi-VN').format(totalLeads);
            
            const avgCpl = totalLeads > 0 ? Math.round(totalSpendFB / totalLeads) : 0;
            const elCpl = document.getElementById('perf-cpl');
            if (elCpl) elCpl.innerText = new Intl.NumberFormat('vi-VN').format(avgCpl) + " ₫";
            ...
            const elFinSpend = document.getElementById('fin-spend');
            if (elFinSpend) elFinSpend.innerText = new Intl.NumberFormat('vi-VN').format(totalCostAll) + " ₫";
            
            const elFinLeads = document.getElementById('fin-leads');
            if (elFinLeads) elFinLeads.innerText = new Intl.NumberFormat('vi-VN').format(totalLeads);
            
            const elFinRev = document.getElementById('fin-revenue');
            if (elFinRev) elFinRev.innerText = new Intl.NumberFormat('vi-VN').format(totalRevenue) + " ₫";
            
            const roas = totalCostAll > 0 ? (totalRevenue / totalCostAll) : 0;
            const elFinRoas = document.getElementById('fin-roas');
            if (elFinRoas) elFinRoas.innerText = roas.toFixed(2) + "x";
        }
    }
...
// CÁC HÀM XỬ LÝ SỐ LIỆU (BẮT BUỘC PHẢI CÓ ĐỂ TRÁNH LỖI)
