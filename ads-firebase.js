/**
 * ADS MODULE V87 (BẢN FINAL TỐI ƯU GIAO DIỆN & NGÔN NGỮ CHUYÊN SÂU)
 * - Đổi "Tiềm Năng LV2" thành "CẦN TỐI ƯU".
 * - Các đề xuất và chẩn đoán được viết lại cực kỳ chi tiết, chỉ đích danh lỗi và cách sửa.
 * - Gộp cột Giá Tin và Giá Đơn (CPA) cho gọn bảng chi tiết.
 * - Bảng Bệnh án hiển thị khi Click với màu nền Navy tương phản cao.
 */
...
function getSystemDiagnosis(spend, cpa, cpm, roas, ctr, freq, cr, thresholds, hasRevenue) {
    ...
    let fails = [];
    if (!cpaOk) fails.push('CPA');
    if (!ctrOk) fails.push('CTR');
    if (!freqOk) fails.push('TẦN SUẤT');
    if (!crOk) fails.push('CHỐT SALE');

    let failCount = fails.length; // Đếm số lỗi của 4 biến số đầu phễu
    let metCount = 5 - failCount - (hasRevenue && !roasOk ? 1 : 0); // Tổng số tiêu chí đạt được
    ...
    if(!freqOk) tooltipList += `<li style="color:#E74C3C"><b>Tần suất (${freq.toFixed(1)} > 3):</b> Bão hòa, cần thay nội dung mới.</li>`;
    else if(freq > 0) tooltipList += `<li style="color:#2ECC71"><b>Tần suất (${freq.toFixed(1)}):</b> Tốt.</li>`;

    if(!ctrOk) tooltipList += `<li style="color:#E74C3C"><b>CTR (${ctr.toFixed(2)}% < 1%):</b> Kém thu hút, khách lướt qua nhiều.</li>`;
    ...
    else if(cpm > 0) tooltipList += `<li style="color:#2ECC71"><b>Giá tin (${formatNumber(cpm)}đ):</b> Giá đạt chuẩn.</li>`;
    ...
    if (hasRevenue && !roasOk && !isLearning) {
        ...
        reason = `Bất kể chỉ số khác ra sao, biên độ lợi nhuận gánh không nổi chi phí quảng cáo (ROAS ${roas.toFixed(2)}x < 2).`;
    ...
    // 4. NHÓM CẦN TỐI ƯU -> Đạt 3/5 (Rớt 2)
        ...
        } else if (fails.includes('CTR') && fails.includes('CPA')) {
            reason = `Ít click (CTR=${ctr.toFixed(2)}%) đẩy Giá thầu lên cao làm Giá Đơn (CPA) đắt.`;
            action = 'Khẩn cấp thay đổi Thumbnail, Tiêu đề hoặc Hook 3s đầu để kéo tỷ lệ nhấp lên.';
        } else if (fails.includes('TẦN SUẤT') && fails.includes('CPA')) {
            reason = `Hiển thị lặp lại tệp cũ (F=${freq.toFixed(1)}) làm chi phí CPM đắt, kéo theo CPA vượt trần.`;
        ...
        } else if (fails.includes('CHỐT SALE') && fails.includes('CPA')) {
            reason = `Giá thầu đắt (CPA cao) cộng thêm bộ phận Sales chốt trượt nhiều (CR=${cr.toFixed(1)}%).`;
            action = 'Khẩn cấp tung Khuyến mãi (Offer) mạnh hơn và rà soát lại quy trình tư vấn khách hàng.';
        } else {
            reason = `Đang bị hụt 2 chỉ số: ${fails.join(' và ')}.`;
            action = `Tập trung phân tích và khắc phục ${fails.join(', ')} để cứu vãn chiến dịch.`;
        }
    }
    // 5. NHÓM MÁY HỌC HOẶC KÉM -> (Rớt 3 hoặc 4)
        ...
        if (isLearning) {
            ...
            if (hasRevenue && roas >= 2) {
                ...
                badgeStyle = 'color:#d93025; font-weight:bold; background:#fce8e6; padding:3px 6px; border-radius:4px; font-size:10px; border: 1px solid #d93025;';
                color = 'rgba(217, 48, 37, 0.7)'; border = '#d93025';
                reason = `Sự thật là bài đang ngáp ngoải và sống nhờ ăn may từ vài đơn to. Phễu đã gãy (${fails.join(', ')}).`;
                action = 'TUYỆT ĐỐI KHÔNG TĂNG NGÂN SÁCH. Giữ chạy vắt kiệt lãi, rớt đơn to là Tắt ngay lập tức.';
            } else {
                label = '❌ CẦN TẮT (Rớt >= 3 đ/k)';
                badgeStyle = 'color:#d93025; font-weight:bold; background:#fce8e6; padding:3px 6px; border-radius:4px; font-size:10px; border: 1px solid #d93025;';
                color = 'rgba(217, 48, 37, 0.7)'; border = '#d93025';
                reason = `Chiến dịch trượt ${failCount}/5 điều kiện đánh giá. Hiệu quả các lớp phễu quá kém.`;
...
function drawChartTrend(companyData) {
    try {
        ...
        const bubbleData = points.map(p => {
            ...
            return {
                x: p.spend, y: p.cpa, // Trục Y hiển thị Giá Đơn (CPA)
            ...
            options: { 
                ...
                plugins: { 
                    ...
                    annotation: {
                        annotations: {
                            line1: {
                                type: 'line', yMin: targetCPA, yMax: targetCPA,
        ...
        if (inputCpl && !inputCpl.value) inputCpl.placeholder = `Auto: ~${Math.round(targetCPA/1000)}k`;
