/**
 * ADS MODULE V81 (STRICT ADSET & TAB 3 FIX)
 * - Xây dựng trên nền tảng code ổn định nhất của user.
 * - Khóa chặt: CHỈ cho phép up file "Nhóm quảng cáo" (Ad Set). Báo lỗi nếu up "Chiến dịch".
 * - Bổ sung mã HTML cho Tab 3 (Biểu đồ xu hướng) để hiển thị đầy đủ.
 */
...
// ==========================================
// BỘ QUÉT FILE - KHÓA CHẶT TÊN NHÓM QUẢNG CÁO
// ==========================================
function parseDataCore(rows) {
    ...
    // Tìm dòng Header (Chỉ quét tối đa 15 dòng đầu cho nhẹ)
    for (let i = 0; i < Math.min(rows.length, 15); i++) {
        ...
        // Chỉ xác nhận Header nếu có Tên Nhóm VÀ Số tiền chi tiêu
        if (hasAdSet && (rowStr.includes("số tiền") || rowStr.includes("amount"))) {
            ...
            row.forEach((cell, idx) => {
                ...
                // Quét cực kỳ nghiêm ngặt để không nhận nhầm cột "Ngân sách nhóm quảng cáo"
                if (txt === "tên nhóm quảng cáo" || txt === "ad set name" || txt === "tên nhóm") colNameIdx = idx;
                else if (txt.includes("tên nhóm") && colNameIdx === -1) colNameIdx = idx;

                if (txt === "số tiền đã chi tiêu (vnd)" || txt === "amount spent (vnd)" || txt === "số tiền đã chi") colSpendIdx = idx;
    ...
    // BÁO LỖI NẾU UP SAI FILE
    if (!hasAdSet) {
        if (hasCampaign) {
            throw new Error("❌ LỖI: Vui lòng xuất báo cáo theo cấp độ NHÓM QUẢNG CÁO! (Bạn đang up file Chiến dịch)");
        }
        throw new Error("❌ LỖI: Không tìm thấy cột 'Tên nhóm quảng cáo'. Vui lòng kiểm tra lại file FB Ads.");
    }
...
function handleFirebaseUpload(e) { 
    ...
    reader.onload = function(e) { 
        try { 
            ...
            // Nếu có lỗi file, sẽ throw từ trong hàm parseDataCore và nhảy xuống catch
            ...
            // In thông báo lỗi màu đỏ rõ ràng
